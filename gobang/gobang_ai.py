...
class game:
    ...
    **Parameters**
    ...
    def move(self, x, y):
        ...
        **Parameters**
        ...
        if self.status == 'Black':
            ...
            # print(self.board)
            self.history.append((self.board[p[0]][p[1]], p[0], p[1]))
            print(self.history)
    ...
    def calc_score(self, ima):
        '''
        To choose the best position for ai, we give every position
        a score. 100 and -100 means ai and player wins, the smaller
        the number, the player are more likely to win.

         **Parameters**
            ima: *list*
                The 2D list recording the hypothetical board.

            last_step: *tuple*
                A tuple recording the kind of piece and its position
        '''

        # if the board has 5 pieces in a line. Then this will lead one
        # party to win

        # The horizontal direction
        for x in range(11):
            for y in range(15):
                if ima[x][y] == 1 and ima[x + 1][y] == 1 \
                        and ima[x + 2][y] == 1 and ima[x + 3][y] == 1 \
                        and ima[x + 4][y] == 1:
                    return -100
                if ima[x][y] == -1 and ima[x + 1][y] == -1 \
                        and ima[x + 2][y] == -1 and ima[x + 3][y] == -1 \
        ...
        for x in range(15):
            for y in range(11):
                if ima[x][y] == 1 and ima[x][y + 1] == 1 \
                        and ima[x][y + 2] == 1 and ima[x][y + 3] == 1 \
                        and ima[x][y + 4] == 1:
                    return -100
                if ima[x][y] == -1 and ima[x][y + 1] == -1 \
                        and ima[x][y + 2] == -1 and ima[x][y + 3] == -1 \
        ...
        for x in range(11):
            for y in range(11):
                if ima[x][y] == 1 and ima[x + 1][y + 1] == 1 \
                        and ima[x + 2][y + 2] == 1 and ima[x + 3][y + 3] == 1 \
                        and ima[x + 4][y + 4] == 1:
                    return -100
                if ima[x][y] == -1 and ima[x + 1][y + 1] == -1 \
                        and ima[x + 2][y + 2] == -1 and \
                        ima[x + 3][y + 3] == -1 and ima[x + 4][y + 4] == -1:
                    return 100

        # From top right to bottom left
        for x in range(11):
            for y in range(11):
                if ima[x + 4][y] == 1 and ima[x + 3][y + 1] == 1 \
                        and ima[x + 2][y + 2] == 1 and ima[x + 1][y + 3] == 1 \
                        and ima[x][y + 4] == 1:
                    return -100
                if ima[x + 4][y] == -1 and ima[x + 3][y + 1] == -1 \
                        and ima[x + 2][y + 2] == -1 and \
                        ima[x + 1][y + 3] == -1 and ima[x][y + 4] == -1:
                    return 100

        # To check if the board has 4 pieces in a line.
        # Because we need 4 position to put pieces, then we only need to
        # check first 11 position.
        ai_4_num = 0
        player_4_num = 0
        ...
        for x in range(11):
            for y in range(15):
                player_cnt = sum([ima[x][y] == 1,
                                  ima[x + 1][y] == 1, ima[x + 2]
                                  [y] == 1,
                                  ima[x + 3][y] == 1, ima[x + 4][y] == 1])
                ai_cnt = sum([ima[x][y] == -1, ima[x + 1][y] == -1, ima[x + 2]
                              [y] == -1,
                              ima[x + 3][y] == -1, ima[x + 4][y] == -1])
        ...
        for x in range(15):
            for y in range(11):
                player_cnt = sum([ima[x][y] == 1, ima[x][y + 1] == 1, ima[x]
                                  [y + 2] == 1,
                                  ima[x][y + 3] == 1, ima[x][y + 4] == 1])
                ai_cnt = sum([ima[x][y] == -1, ima[x][y + 1] == -1, ima[x]
                              [y + 2] == -1, ima[x][y + 3] == -1,
                              ima[x][y + 4] == -1])
                if player_cnt == 4 and ai_cnt == 0:
                    player_4_num += 1
        ...
        for x in range(11):
            for y in range(11):
                player_cnt = sum([ima[x][y] == 1, ima[x + 1][y + 1] == 1,
                                  ima[x + 2]
                                  [y + 2] == 1, ima[x + 3][y + 3] == 1,
                                  ima[x + 4][y + 4] == 1])
                ai_cnt = sum([ima[x][y] == -1, ima[x + 1][y + 1] == -1,
                              ima[x + 2]
                              [y + 2] == -1, ima[x + 3][y + 3] == -1,
                              ima[x + 4][y + 4] == -1])
                if player_cnt == 4 and ai_cnt == 0:
        ...
        for x in range(11):
            for y in range(11):
                player_cnt = sum([ima[x + 4][y] == 1,
                                  ima[x + 3][y + 1] == 1, ima[x + 2]
                                  [y + 2] == 1, ima[x + 1][y + 3] == 1,
                                  ima[x][y + 4] == 1])
                ai_cnt = sum([ima[x + 4][y] == -1,
                              ima[x + 3][y + 1] == -1, ima[x + 2]
                              [y + 2] == -1,
                              ima[x + 1][y + 3] == -1, ima[x][y + 4] == -1])
                if player_cnt == 4 and ai_cnt == 0:
                    player_4_num += 1
                if ai_cnt == 4 and player_cnt == 0:
                    ai_4_num += 1
        # To check if there are two sides that both have 4 pieces
        if player_4_num >= 2:
            return -90
        elif ai_4_num >= 2 and player_4_num == 0:
            return 90

        # To check if the board has 3 pieces in a line.
        player_3d_num = 0
        ai_3d_num = 0

        # From four directions check if there is a layout like xooox
        # x is the vacancy, o is the existed piece.
        for x in range(11):
            for y in range(15):
                if ima[x][y] == 0 and ima[x + 1][y] == 1 and \
                        ima[x + 2][y] == 1 and ima[x + 3][y] == 1 and \
                        ima[x + 4][y] == 0:
                    player_3d_num += 1
                if ima[x][y] == 0 and \
                        ima[x + 1][y] == -1 and ima[x + 2][y] == -1 and \
                        ima[x + 3][y] == -1 and ima[x + 4][y] == 0:
                    ai_3d_num += 1

        for x in range(15):
            for y in range(11):
                if ima[x][y] == 0 and ima[x][y + 1] == 1 and \
                        ima[x][y + 2] == 1 and ima[x][y + 3] == 1 and \
                        ima[x][y + 4] == 0:
                    player_3d_num += 1
                if ima[x][y] == 0 and ima[x][y + 1] == -1 and \
                        ima[x][y + 2] == -1 and ima[x][y + 3] == -1 and \
        ...
        for x in range(11):
            for y in range(11):
                if ima[x][y] == 0 and ima[x + 1][y + 1] == 1 and \
                        ima[x + 2][y + 2] == 1 and ima[x + 3][y + 3] == 1 and \
                        ima[x + 4][y + 4] == 0:
                    player_3d_num += 1
                if ima[x][y] == 0 and ima[x + 1][y + 1] == -1 and \
                        ima[x + 2][y + 2] == -1 and \
        ...
        for x in range(11):
            for y in range(11):
                if ima[x + 4][y] == 0 and \
                        ima[x + 3][y + 1] == 1 and ima[x + 2][y + 2] == 1 and \
                        ima[x + 1][y + 3] == 1 and ima[x][y + 4] == 0:
                    player_3d_num += 1
                if ima[x + 4][y] == 0 and ima[x + 3][y + 1] == -1 and \
                        ima[x + 2][y + 2] == -1 and \
                        ima[x + 1][y + 3] == -1 and ima[x][y + 4] == 0:
                    ai_3d_num += 1

        # From four directions check if there is a layout like xoxoox or xooxox
        for x in range(10):
            for y in range(15):
                if ima[x][y] == 0 and ima[x + 1][y] == 1 and \
                    ((ima[x + 2][y] == 1) ^ (ima[x + 3][y] == 1)) and \
                    ima[x + 2][y] != -1 and ima[x + 3][y] != -1 and \
                        ima[x + 4][y] == 1 and ima[x + 5][y] == 0:
                    player_3d_num += 1
                if ima[x][y] == 0 and ima[x + 1][y] == -1 and \
                    ((ima[x + 2][y] == -1) ^ (ima[x + 3][y] == -1)) and \
                    ima[x + 2][y] != 1 and ima[x + 3][y] != 1 and \
        ...
        for x in range(15):
            for y in range(10):
                if ima[x][y] == 0 and ima[x][y + 1] == 1 and \
                    ((ima[x][y + 2] == 1) ^ (ima[x][y + 3] == 1)) and \
                    ima[x][y + 2] != -1 and ima[x][y + 3] != -1 and \
                        ima[x][y + 4] == 1 and ima[x][y + 5] == 0:
                    player_3d_num += 1
                if ima[x][y] == 0 and ima[x][y + 1] == -1 and \
                    ((ima[x][y + 2] == -1) ^ (ima[x][y + 3] == -1)) and \
                    ima[x][y + 2] != 1 and ima[x][y + 3] != 1 and \
        ...
        for x in range(10):
            for y in range(10):
                if ima[x][y] == 0 and ima[x + 1][y + 1] == 1 and \
                    ((ima[x + 2][y + 2] == 1) ^ (ima[x + 3][y + 3] == 1)) and \
                    ima[x + 2][y + 2] != -1 and ima[x + 3][y + 3] != -1 and \
                        ima[x + 4][y + 4] == 1 and ima[x + 5][y + 5] == 0:
                    player_3d_num += 1
                if ima[x][y] == 0 and ima[x + 1][y + 1] == -1 and \
                    ((ima[x + 2][y + 2] == -1) ^ (ima[x + 3][y + 3] == -1)) \
                    and ima[x + 2][y + 2] != 1 and ima[x + 3][y + 3] != 1 \
                        and ima[x + 4][y + 4] == -1 and ima[x + 5][y + 5] == 0:
                    ai_3d_num += 1

        for x in range(10):
            for y in range(10):
                if ima[x + 5][y] == 0 and ima[x + 4][y + 1] == 1 and \
                    ((ima[x + 3][y + 2] == 1) ^ (ima[x + 2][y + 3] == 1)) \
                    and ima[x + 3][y + 2] != -1 and ima[x + 2][y + 3] != -1 \
                        and ima[x + 1][y + 4] == 1 and ima[x][y + 5] == 0:
                    player_3d_num += 1
                if ima[x + 5][y] == 0 and ima[x + 4][y + 1] == -1 and \
                    ((ima[x + 3][y + 2] == -1) ^ (ima[x + 2][y + 3] == -1)) \
                    and ima[x + 3][y + 2] != 1 and ima[x + 2][y + 3] != 1 \
        ...
        # To check if there are two sides that one has 4 pieces
        # the other has 3 pieces
        ...
        # To check if there is four pieces in a line
        if player_4_num:
            return -70
        elif ai_4_num:
            return 70

        # To check if there are two sides that both have 3 pieces
        ...
        # To check if there are three pieces in a line
        ...
        # In other conditions, we give a score according to how close
        # the position is to the center, and how many pieces around this
        # postion
                ...
                if ima[x][y] == 1:
                    ...
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 14 and 0 <= y0 <= 14 and \
                                    ima[x0][y0] != 0:
                                around_cnt += 1
                    player_score_num += score_by_num_around[around_cnt] - abs(
                        x - 7) - abs(y - 7)
                    player_cnt += 1
                if ima[x][y] == -1:
                    around_cnt = 0
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 14 and 0 <= y0 <= 14 and \
                                    ima[x0][y0] != 0:
                                around_cnt += 1
                    ai_score_num += score_by_num_around[around_cnt] - abs(
                        x - 7) - abs(y - 7)
                    ai_cnt += 1
    ...
    def find_position(self):
        '''
        Acoording to the score, decide which position should AI choose
        '''

        # If a position has existed pieces around it, then this position
        # is optional.
        # Firstly we need to find which positions are optional
        ope_list = set()
        for x in range(15):
            for y in range(15):
                if self.board[x][y] != 0:
        ...
        position1 = {}
        position2 = {}
        # Assume AI put pieces at one of the optional position, find all
        # positions that player could use
        for cell in ope_list:
            ima = copy.deepcopy(self.board)
            ima[cell[0]][cell[1]] = -1
            ope_list_new = set()
            SCORE = []
            for p in range(15):
                for q in range(15):
                    if ima[p][q] != 0:
                        for p0 in range(p - 1, p + 2):
                            for q0 in range(q - 1, q + 2):
                                if 0 <= p0 <= 14 and 0 <= q0 <= 14 and \
                                        ima[p0][q0] == 0:
                                    ope_list_new.add((p0, q0))
            # print('ope_list_new', ope_list_new)
            # calculate the score, and choose the postition
            for cell_0 in ope_list_new:
                global ima_new
                ima_new = copy.deepcopy(ima)
                ima_new[cell_0[0]][cell_0[1]] = 1
                score = self.calc_score(ima_new)
                SCORE.append(score)
                # if cell[0] == 9 and cell[1] == 6:
                #     print('cell[0],cell[1]', cell[0], cell[1])
                #     print('cell_0', cell_0[0], cell_0[1])
                #     print('score', score)

            Score1 = min(SCORE)
            Score2 = max(SCORE)
            position1[(cell[0], cell[1])] = Score1
            position2[(cell[0], cell[1])] = Score2
            # print('Position1', position1)
            # print('Position2', position2)
        if abs(min(position1.values())) > abs(max(position2.values())):
            for key, value in position1.items():
                if(value == max(position1.values())):
                    # print('key', key)
                    return key
        else:
            for key, value in position2.items():
                if(value == max(position2.values())):
                    # print('key', key)
                    return key
    ...
    def MOVE(self, position):
        ...
        **Parameters**
        ...
        if last_move[0] == 1:
            ...
            piece_color = (255, 255, 255)
            a = self.border_width + self.unit * last_move[1]
            b = self.border_width + self.unit * last_move[2]
            pygame.draw.circle(self.screen, piece_color, [
                a, b], int(self.unit / 2.5))
